...
class Projects extends Security_Controller {
    ...
    private function _make_timesheet_row($data, $custom_fields) {
        ...
        $project_title = anchor(get_uri("projects/task_list/" . $data->project_id), $data->project_title);
        ...
        if ($file_info) {
            ...
            $view_data['modal_fixes'] = '
                <style>
                    ...
                    /* Hide the actual modal navigation buttons */
                    .app-modal-next-button,
                    .app-modal-prev-button,
                    .app-modal-navigation-buttons {
                        display: none !important;
                        visibility: hidden !important;
                    }
                    /* Hide when modal is open */
                    .modal-open .app-modal-next-button,
                    .modal-open .app-modal-prev-button,
                    body.modal-open .app-modal-next-button,
                    body.modal-open .app-modal-prev-button {
                        display: none !important;
                        visibility: hidden !important;
                    }
                    /* Target any modal navigation elements */
                    [class*="modal-nav"],
                    [class*="modal-next"],
                    [class*="modal-prev"],
                    [class*="app-modal-nav"] {
                ...
                <script>
                    $(document).ready(function() {
                        ...
                        // Hide navigation buttons when modal opens
                        $("body").on("shown.bs.modal", ".modal", function() {
                            $(".app-modal-next-button, .app-modal-prev-button").hide();
                            $("[class*=\'modal-nav\'], [class*=\'modal-next\'], [class*=\'modal-prev\']").hide();
                        });
                        
                        // Keep them hidden when modal closes (optional)
                        $("body").on("hidden.bs.modal", ".modal", function() {
                            // Uncomment if you want to show them again after modal closes
    ...
    function file_modal_form() {
        ...
        $project_id = $this->request->getPost('project_id') ? $this->request->getPost('project_id') : $view_data['model_info']->project_id;
        $folder_id = $this->request->getPost('folder_id') ? $this->request->getPost('folder_id') : 0;
    ...
    /* explore method for folder navigation */
    function explore($folder_id = 0, $page = 1, $view_from = "", $project_id = 0) {
        $this->init_project_permission_checker($project_id);
    ...
    function create_folder() {
        $project_id = $this->request->getPost('project_id');
        $folder_name = $this->request->getPost('name');
        $parent_folder_id = $this->request->getPost('parent_folder_id') ?: 0;
        ...
        if (!$folder_name) {
            echo json_encode(array("success" => false, "message" => app_lang('folder_name_required')));
            return;
        }
        
        // For now, simulate folder creation
        // You can implement actual folder creation logic here if you have a folders table
        $folder_data = array(
            "name" => $folder_name,
            "project_id" => $project_id,
        ...
        // If you have a Project_folders_model, use it here:
        // $folder_id = $this->Project_folders_model->ci_save($folder_data);
        
        echo json_encode(array(
            "success" => true, 
            "message" => app_lang('folder_created_successfully')
        ));
    }
    ...
    function delete_folder() {
        ...
        if (!$folder_id) {
            echo json_encode(array("success" => false, "message" => app_lang('invalid_folder')));
            return;
        }
        
        // Get folder info to check project permissions
        // $folder_info = $this->Project_folders_model->get_one($folder_id);
        // $this->init_project_permission_checker($folder_info->project_id);
        ...
        // $has_files = $this->Project_files_model->get_details(array("folder_id" => $folder_id))->getNumRows();
        // $has_subfolders = $this->Project_folders_model->get_details(array("parent_folder_id" => $folder_id))->getNumRows();
        
        // if ($has_files || $has_subfolders) {
        //     echo json_encode(array("success" => false, "message" => app_lang('folder_not_empty')));
        //     return;
        // }
        
        // Delete folder
        // $delete_result = $this->Project_folders_model->delete($folder_id);
        
        echo json_encode(array(
            "success" => true,
            "message" => app_lang('folder_deleted_successfully')
        ));
    }
    ...
    private function get_file_icon($extension) {
        ...
        return isset($icon_map[$extension]) ? $icon_map[$extension] : 'fa-file';
    }
    ...
    private function _make_file_row($data, $custom_fields) {
        ...
        $description = "<div class='float-start text-wrap'>" .
                js_anchor(remove_file_prefix($data->file_name), array(
                    'title' => remove_file_prefix($data->file_name), 
                    "data-toggle" => "app-modal", 
                    "data-sidebar" => "1", 
                    "data-url" => get_uri("projects/view_file/" . $data->id),
                    "data-modal-lg" => "1",
                    "data-modal-title" => remove_file_prefix($data->file_name)
                ));
    ...
    public function task_list($project_id = 0, $tab = "")
    {
        validate_numeric_value($project_id);
        $this->init_project_permission_checker($project_id);
        ...
        // Get all tasks (flat array) - FIXED: Use get_details instead of get_hierarchical_tasks
        $tasks = $Tasks_model->get_details(array("project_id" => $project_id))->getResult();
        
        // Get team members for assignment dropdown
        $team_members = $Users_model->get_details(array("user_type" => "staff", "status" => "active"))->getResult();
        ...
        $existing_tasks = $Tasks_model->get_details(array("project_id" => $project_id))->getResult();
        $parent_tasks_dropdown = array(array("id" => "0", "text" => "- Main Task -"));
        foreach ($existing_tasks as $task) {
            $indent = str_repeat("&nbsp;&nbsp;", $task->task_level ?? 0);
            $parent_tasks_dropdown[] = array("id" => $task->id, "text" => $indent . $task->title);
        }
        
        $view_data['project_info'] = $project_info;
        $view_data['project_id'] = $project_id;
        $view_data['tasks'] = $tasks;
        $view_data['team_members_dropdown'] = $team_members_dropdown;
        $view_data['parent_tasks_dropdown'] = $parent_tasks_dropdown;
        
        // Check if this is an AJAX request (for tab loading)
        if ($this->request->isAJAX() || $this->request->getGet('ajax')) {
            $view_data['page_type'] = "tab";
            return $this->template->view("project_management/task_list", $view_data);
        } else {
            $view_data['page_type'] = "full";
    ...
    public function upload_task_image()
    {
        $task_id = $this->request->getPost('task_id');
        
        if (!$task_id) {
            echo json_encode(array("success" => false, "message" => "Invalid task"));
            return;
        }
        ...
        if ($file->isValid() && !$file->hasMoved()) {
            ...
            $file->move(FCPATH . 'files/task_images/', $newName);
            
            // Update task with image
            $task_info = $Tasks_model->get_one($task_id);
            $images = $task_info->images ? json_decode($task_info->images, true) : array();
            $images[] = $newName;
            
            $data = array('images' => json_encode($images));
            $Tasks_model->ci_save($data, $task_id);
    ...
    public function update_task_hierarchy()
    {
        ...
        $new_level = $this->request->getPost('new_level');
        $new_position = $this->request->getPost('new_position');
        ...
        // Validate level (max 10 levels)
        if ($new_level > 10) {
            echo json_encode(array("success" => false, "message" => "Maximum 10 levels allowed"));
            return;
        }
        ...
        $result = $Tasks_model->ci_save($data, $task_id);
        
        if ($result) {
            echo json_encode(array("success" => true, "message" => "Task hierarchy updated successfully"));
        } else {
            echo json_encode(array("success" => false, "message" => "Failed to update task hierarchy"));
        }
    }
