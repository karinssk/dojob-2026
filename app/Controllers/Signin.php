...
class Signin extends App_Controller {
    ...
    function authenticate() {
        ...
        $user_info = $this->Users_model->authenticate($email, $password);
        if (!$user_info) {
        ...
        if ($remember && $user_info) {
            $this->_set_remember_me_cookie($user_info);
        }
    ...
    /**
     * Set remember me cookie for 3 months
     */
    private function _set_remember_me_cookie($user_info) {
        $cookie_data = array(
            'name' => 'remember_me',
            'value' => $user_info->id . '|' . md5($user_info->password),
            'expire' => 60 * 60 * 24 * 90, // 90 days (3 months)
            'httponly' => true, // Security: prevent JavaScript access
            'secure' => $this->request->isSecure(), // Use secure flag if HTTPS
            'samesite' => 'Lax'
        );
        $this->response->setCookie($cookie_data);
    }
