...
class App_Controller extends Controller {
    ...
    public $Storyboards_model;
    public $Sub_storyboard_projects_model;
    ...
    public function __construct() {
        ...
        //load helpers
        helper(array('url', 'file', 'form', 'language', 'text', 'general', 'date_time', 'app_files', 'widget', 'activity_logs', 'currency', 'reports'));
     ...
     */
    private function _extendSessionOnActivity() {
        $lastActivity = $this->session->get('last_activity');
        $currentTime = time();
        
        // If no last activity recorded or enough time has passed (1 hour), extend session
        if (!$lastActivity || ($currentTime - $lastActivity) > 3600) {
            // Update last activity timestamp
            $this->session->set('last_activity', $currentTime);
            
            // Extend session expiration (3 months from now)
            $this->session->set('session_extended_until', $currentTime + 7776000);
            
            // Log session extension for debugging (optional)
            log_message('info', 'Session extended for user ID: ' . $this->Users_model->login_user_id() . ' until: ' . date('Y-m-d H:i:s', $currentTime + 7776000));
        }
    }
}
