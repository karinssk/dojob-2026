...
class Tasks extends Security_Controller {
    ...
    function all_tasks_kanban_data() {
        ...
        if ($column_id) {
            ...
            $this->_attach_task_comment_files($view_data["tasks"]);
    ...
    private function _attach_task_comment_files(&$tasks = array()) {
        ...
        foreach ($tasks as $task) {
            if (is_object($task) && isset($task->id)) {
                ...
                $task->all_comment_files_array = array();
                $task->all_comment_files = json_encode(array());
            }
        }
        ...
        foreach ($comment_files as $comment_row) {
            if (!$comment_row->files) {
                continue;
            }

            $files = @unserialize($comment_row->files);
            if (!$files || !is_array($files)) {
                continue;
            }

            foreach ($files as $file) {
                if (!is_array($file) || !get_array_value($file, "file_name")) {
                    continue;
                }

                $file["comment_id"] = $comment_row->id;
                $file["comment_created_at"] = $comment_row->created_at;
                $files_map[$comment_row->task_id][] = $file;
            }
        }

        foreach ($tasks as $task) {
            $task_files = get_array_value($files_map, $task->id);
            if (!$task_files) {
                $task->all_comment_files_array = array();
                $task->all_comment_files = json_encode(array());
                continue;
            ...
            $task_files = array_values($task_files);
            $task->all_comment_files_array = $task_files;
            $encoded_files = json_encode($task_files);
            $task->all_comment_files = $encoded_files !== false ? $encoded_files : json_encode(array());
        }
    }
