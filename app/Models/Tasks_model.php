...
class Tasks_model extends Crud_model {
    ...
    function get_kanban_details($options = array()) {
        ...
        LEFT JOIN (
            SELECT pc.*, ROW_NUMBER() OVER (PARTITION BY pc.task_id ORDER BY pc.created_at DESC) as rn
            FROM $project_comments_table pc
            WHERE pc.deleted=0 AND pc.task_id != 0
        ) AS last_comment ON last_comment.task_id = $tasks_table.id AND last_comment.rn = 1
        LEFT JOIN $users_table AS comment_user ON comment_user.id = last_comment.created_by
        $extra_left_join
        WHERE $tasks_table.deleted=0 $where $custom_fields_where 
