...
class Users_model extends Crud_model {
    ...
    function authenticate($email, $password) {
        ...
        if ($result_count === 1) {
            ...
            foreach ($result->getResult() as $user_info) {
                $verified_user = $this->verify_password($user_info, $password);
                if ($verified_user) {
    ...
    private function verify_password($user_info, $password) {
        ...
        if ($user_info->password && (strlen($user_info->password) === 60 && password_verify($password, $user_info->password)) || $user_info->password === md5($password)) {
            ...
            if ($this->_client_can_login($user_info) !== false) {
                ...
                return $user_info; // Return user info instead of just true
            }
        }
        return false;
    }
    ...
    function login_user_id() {
        ...
        $remember_cookie = $request->getCookie('remember_me');
        
        if ($remember_cookie) {
            $cookie_parts = explode('|', $remember_cookie);
            if (count($cookie_parts) == 2) {
                $user_id = $cookie_parts[0];
                $password_hash = $cookie_parts[1];
                
                // Verify the cookie
                $user = $this->get_one($user_id);
                if ($user && $user->status === 'active' && $user->deleted == 0 && md5($user->password) === $password_hash) {
                    // Re-login the user automatically
    ...
    function get_user_info_by_email($email) {
        ...
        $result = $this->db_builder->getWhere(array('email' => $email, 'status' => 'active', 'deleted' => 0));
    ...
    function sign_out() {
        ...
        if ($request->getCookie('remember_me')) {
