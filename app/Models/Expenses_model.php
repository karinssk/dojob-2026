...
class Expenses_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $month_filter = $this->_get_clean_value($options, "month_filter");
        if ($month_filter) {
            // Month filter format: YYYY-MM (e.g., 2024-03)
            $where .= " AND DATE_FORMAT($expenses_table.expense_date, '%Y-%m') = '$month_filter'";
        }
