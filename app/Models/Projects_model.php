...
class Projects_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        $is_storyboard = $this->_get_clean_value($options, "is_storyboard");
        if ($is_storyboard !== null) {
            if ($is_storyboard == 1) {
                $where .= " AND $projects_table.is_storyboard = 1";
            } else if ($is_storyboard == 0) {
                $where .= " AND ($projects_table.is_storyboard = 0 OR $projects_table.is_storyboard IS NULL)";
            }
        }
