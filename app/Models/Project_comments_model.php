...
class Project_comments_model extends Crud_model {
    ...
    function get_files_for_tasks($task_ids = array()) {
        ...
        $task_ids_string = implode(",", $clean_ids);
        $project_comments_table = $this->db->prefixTable('project_comments');

        $sql = "SELECT id, task_id, created_at, files
                FROM $project_comments_table
                WHERE deleted=0 AND task_id IN ($task_ids_string) AND files!=''
