...
class Attendance_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($user_ids && is_array($user_ids)) {
            ...
            $where .= " AND $attendnace_table.user_id IN($user_ids_string)";
        }
        ...
        $sql = "SELECT $attendnace_table.*,  CONCAT($users_table.first_name, ' ',$users_table.last_name) AS created_by_user, $users_table.image as created_by_avatar, $users_table.job_title as user_job_title
        FROM $attendnace_table
        INNER JOIN $users_table ON $users_table.id = $attendnace_table.user_id
        WHERE $attendnace_table.deleted=0 AND $users_table.deleted=0 AND $users_table.status='active' AND $users_table.user_type='staff' $where
