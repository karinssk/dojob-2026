...
foreach ($tasks as $task) {
    ...
    if (isset($task->last_comment_description) && $task->last_comment_description) {
        ...
        if ($last_comment_files && $last_comment_files_json !== "") {
            ...
            if (!empty($image_files_data)) {
                ...
                $comment_images_html .= "<div class='mt5 kanban-image-hover-wrapper' data-task-id='$task->id' data-all-files='" . $last_comment_files_json . "'>";
                $comment_images_html .= "<img src='" . $primary_image["url"] . "' class='kanban-comment-image' data-task-id='$task->id' data-all-files='" . $last_comment_files_json . "' data-file-index='" . $primary_image["index"] . "' style='width: 274px; height: 240px; object-fit: cover; border-radius: 3px;' alt='" . $primary_image["title"] . "' />";
    ...
    $view_images_button = "";
    if ($last_comment_files) {
        $total_images = 0;
        foreach ($last_comment_files as $file) {
            if (isset($file['file_name'])) {
        ...
        if ($total_images > 0 && $last_comment_files_json !== "") {
            $view_images_button = "<div class='mt5'><button class='btn btn-sm btn-outline-primary view-images-btn' data-task-id='$task->id' data-all-files='" . $last_comment_files_json . "'><i data-feather='image' class='icon-14 mr5'></i>" . app_lang('view_images') . " ($total_images)</button></div>";
        }
    }
