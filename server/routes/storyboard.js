...
router.post("/export-data", async (req, res) => {
  ...
  try {
    ...
    if (projectRows.length === 0) {
      ...
      console.log(
        "🔍 Project exists but deleted?:",
        allProjectRows.length,
        allProjectRows
      );
      ...
      if (storyboardCheck[0].count > 0) {
        console.log(
          "🔍 Found storyboards for non-existent project, creating virtual project"
        );
    ...
    storyboardRows.forEach((storyboard) => {
      console.log(
        `🔍 DEBUG: Processing storyboard ${storyboard.id}, frame data:`,
        {
          has_frame: !!storyboard.frame,
          frame_length: storyboard.frame ? storyboard.frame.length : 0,
          frame_preview: storyboard.frame
            ? storyboard.frame.substring(0, 100)
            : null,
        }
      );
      ...
      if (storyboard.frame) {
        try {
          ...
          console.log(
            `🖼️ SUCCESS: Regex parser worked for storyboard ${storyboard.id}:`,
            frameData
          );
        } catch (e) {
          console.log(
            `⚠️ Regex parser failed for storyboard ${storyboard.id}, trying JSON...`
          );
          ...
          try {
            ...
            console.log(
              `🖼️ SUCCESS: JSON parse worked for storyboard ${storyboard.id}:`,
              frameData
            );
          } catch (e2) {
            console.warn(
...
router.post("/export-png", async (req, res) => {
  try {
    ...
    if (preserve_image_size) {
      ...
      for (const storyboard of storyboardRows) {
        ...
        if (frameData && frameData.file_name) {
          try {
            ...
            console.log(
              `📐 Image ${frameData.file_name}: ${image.width}x${image.height}`
            );
          } catch (err) {
      ...
      console.log(
        `📐 Maximum image dimensions found: ${maxImageWidth}x${maxImageHeight}`
      );
    }
    ...
    if (preserve_image_size && maxImageWidth > 0 && maxImageHeight > 0) {
      ...
      sceneHeight = 1000;
      console.log(
        `📐 Using standard A4 canvas size: ${canvasWidth}x${canvasHeight}`
      );
    }
    ...
    for (let i = 0; i < storyboardRows.length; i++) {
      ...
      if (frameData && frameData.file_name) {
        try {
          ...
          console.log(
            `🖼️ Loading image for shot ${storyboard.shot}: ${frameData.file_name}`
          );
          ...
          console.log(
            `📐 Scaled to fit: ${Math.round(drawWidth)}x${Math.round(
              drawHeight
            )} at (${Math.round(drawX)}, ${Math.round(drawY)})`
          );
          ...
          ctx.drawImage(
            ...
            0,
            image.width,
            image.height, // Source: entire image
            drawX,
            drawY,
            drawWidth,
            drawHeight // Destination: scaled size
          );
          ...
          console.warn(
            `Could not load image for storyboard ${storyboard.id}:`,
            err.message
          );
          // Draw placeholder
...
router.post("/export-png-exact", async (req, res) => {
  try {
    ...
    console.log(
      "🖼️ Exporting storyboard as 800x800px PNG with simple layout:",
      {
        project_id,
        sub_project_id,
        selected_scenes: selected_scenes.length,
    ...
    // Multi-page layout: 9 scenes per page (3x3 grid)
    ...
    const cols = 3;
    const rowsPerPage = 3; // 3x3 = 9 scenes per page
    const scenesPerPage = cols * rowsPerPage;
    const padding = 50;
    const gap = 40;

    // Calculate number of pages needed
    const totalPages = Math.ceil(storyboardRows.length / scenesPerPage);

    console.log(`📐 Multi-page export configuration:`);
    console.log(`   - Total scenes: ${storyboardRows.length}`);
    console.log(
      `   - Scenes per page: ${scenesPerPage} (${cols}x${rowsPerPage})`
    );
    console.log(`   - Total pages: ${totalPages}`);

    // Single page canvas size (always 3x3)
    const canvasWidth = padding * 2 + cardWidth * cols + gap * (cols - 1);
    const canvasHeight =
      padding * 2 + cardHeight * rowsPerPage + gap * (rowsPerPage - 1);

    console.log(`   - Page size: ${canvasWidth}x${canvasHeight}px`);

    // If only one page, export as single PNG
    if (totalPages === 1) {
      console.log(`📄 Single page export (${storyboardRows.length} scenes)`);
      ...
      for (let i = 0; i < storyboardRows.length; i++) {
        ...
        if (frameData && frameData.file_name) {
          try {
            ...
            console.log(
              `🖼️ Loading image for shot ${storyboard.shot}: ${frameData.file_name} (${image.width}x${image.height})`
            );
            ...
            console.log(
              `✅ Drew image at 800x800px: scaled to ${Math.round(
                drawWidth
              )}x${Math.round(drawHeight)}`
            );
          } catch (err) {
            console.warn(
              `Could not load image for shot ${storyboard.shot}:`,
              err.message
            );
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="storyboard_800x800_${project.title.replace(
          /[^a-zA-Z0-9]/g,
          "_"
        )}_${Date.now()}.png"`
      );
      ...
      console.log("✅ Single page storyboard PNG exported successfully");
    } else {
      // Multi-page export - create ZIP with multiple PNG files
      console.log(`📦 Multi-page export (${totalPages} pages)`);
      ...
      res.setHeader(
        ...
        `attachment; filename="storyboard_${project.title.replace(
          /[^a-zA-Z0-9]/g,
          "_"
        )}_${totalPages}_pages_${Date.now()}.zip"`
      );
      ...
      for (let pageNum = 0; pageNum < totalPages; pageNum++) {
        ...
        // Calculate scene range for this page
        const startIdx = pageNum * scenesPerPage;
        const endIdx = Math.min(
          startIdx + scenesPerPage,
          storyboardRows.length
        );
        const scenesOnThisPage = endIdx - startIdx;

        console.log(
          `   - Scenes ${
            startIdx + 1
          } to ${endIdx} (${scenesOnThisPage} scenes)`
        );

        // Draw scenes for this page
        for (let i = 0; i < scenesOnThisPage; i++) {
          const sceneIdx = startIdx + i;
          const storyboard = storyboardRows[sceneIdx];
          const col = i % cols;
          const row = Math.floor(i / cols);
        ...
        // Add to ZIP archive
        const pageFilename = `page_${String(pageNum + 1).padStart(2, "0")}.png`;
        archive.append(pageBuffer, { name: pageFilename });
      }
      ...
      console.log(
        `✅ Multi-page storyboard exported successfully: ${totalPages} pages`
      );
    }

    // Note: Success logs are already handled in each branch above
  } catch (error) {
...
router.get("/test-parser", (req, res) => {
  ...
  const testData =
    'a:1:{s:9:"file_name";s:29:"68d7c442e6652_S__27172875.jpg";}';
    ...
    res.json({
      ...
      frame_url:
        result && result.file_name
          ? `/files/storyboard_frames/${result.file_name}`
          : null,
    });
  } catch (error) {
